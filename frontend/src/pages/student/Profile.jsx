...
import { User, Mail, Calendar, UserCircle, BookOpen, Edit2, X, Check } from 'lucide-react';
    ...
    <Layout>
      <div className="space-y-8">
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          <div className="border-b border-gray-200 bg-gray-50/80 px-6 py-6">
            <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-4">
                <StudentAvatar student={profile} size={80} />
                <div>
                  <h2 className="text-xl font-semibold text-gray-900">{profile?.user?.name}</h2>
                  <p className="text-sm text-gray-500">
                    Student {profile?.class?.name ? `· ${profile.class.name}` : ''}
                  </p>
                </div>
              </div>
              {!editing ? (
          ...
          <div className="p-6">
            {editing ? (
              <form id="student-profile-form" onSubmit={handleSubmit} className="max-w-md space-y-4">
                <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full rounded-lg border border-gray-300 py-2.5 px-4 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20"
                  />
                </div>
                <div>
              ...
              <dl className="grid gap-4 sm:grid-cols-1">
                ...
                <div className="flex items-center gap-3 rounded-lg border border-gray-100 bg-gray-50/50 px-4 py-3">
                  ...
                  <div>
                    <dt className="text-xs font-medium uppercase tracking-wider text-gray-500">Class</dt>
                    <dd className="mt-0.5 font-medium text-gray-900">{profile?.class?.name || '—'}</dd>
                  </div>
                </div>
                <div className="flex items-center gap-3 rounded-lg border border-gray-100 bg-gray-50/50 px-4 py-3">
                  <Calendar className="h-5 w-5 text-gray-400" />
                  <div>
                    <dt className="text-xs font-medium uppercase tracking-wider text-gray-500">Date of birth</dt>
                ...
                <div className="flex items-center gap-3 rounded-lg border border-gray-100 bg-gray-50/50 px-4 py-3">
                  ...
                  <div>
                    ...
                    <dd className="mt-0.5 font-medium text-gray-900 capitalize">{profile?.gender || '—'}</dd>
                  </div>
                </div>
              </dl>
            )}
